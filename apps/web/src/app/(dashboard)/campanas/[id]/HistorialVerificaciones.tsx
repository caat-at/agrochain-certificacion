            ...
            <div key={v.id} className="px-4 py-3 space-y-2">
              ...
              {expandidoHash === v.id && (
                <div className="space-y-2">
                  {v.detalles.length === 0 && (
                    <p className="text-[11px] text-gray-400 px-1">No hay registros completos para verificar.</p>
                  )}
                  {v.detalles.map((d) => (
                    <div key={d.id} className={`rounded-lg border px-3 py-2.5 space-y-1.5 text-[11px] ${
                      d.resultado === "FALLA" ? "border-red-200 bg-red-50" : "border-gray-100 bg-gray-50"
                    }`}>
                      <div className="flex items-center justify-between">
                        <span className="font-semibold text-gray-600">{d.etiquetaRegistro ?? "REG-???"}</span>
                        {d.resultado === "OK"
                          ? <span className="text-emerald-600 font-semibold">✓ OK</span>
                          : <span className="text-red-600 font-semibold">⚠ FALLA</span>}
                      </div>
                      <div>
                        <p className="text-gray-400 mb-0.5">Hash guardado:</p>
                        <p className="font-mono text-gray-700 break-all">{d.hashGuardado}</p>
                      </div>
                      <div>
                        <p className="text-gray-400 mb-0.5">Hash recalculado:</p>
                        <p className={`font-mono break-all ${d.resultado === "OK" ? "text-gray-700" : "text-red-600"}`}>
                          {d.hashCalculado}
                        </p>
                      </div>
                    </div>
